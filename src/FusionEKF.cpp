...
FusionEKF::FusionEKF() {
  ...
  ekf_.P_ << 1,  0,   0,   0,
             ...
             0,   0,   1000, 0,
             0,   0,   0,   1000;
std::cout<<"End of method FusionEKF"<<std::endl<<std::flush;
}
...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  //set dt and calculate dt2, 3 and 4 for covariance matrix
  double dt = (measurement_pack.timestamp_ - previous_timestamp_);
  dt /= 1000000.0; // dt in seconds
  double dt_2 = dt * dt;
  double dt_3 = dt_2 * dt;
  double dt_4 = dt_3 * dt;
  previous_timestamp_ = measurement_pack.timestamp_;
  
  //set transition matrix with time (dt)
